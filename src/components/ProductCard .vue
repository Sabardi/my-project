<template>
    <section class="bg-white">
        <!-- bookmark button -->
        <button class="flex justify-end w-full p-2" @click="toggleBookmark(product.id)">
            <span v-if="isBookmarked(product.id)">
        ...
        <a :href="product.id">
            <!-- image -->
            <img :src="product.image" class="w-full h-1/2">
            <!-- footer -->
            <section class="p-6 text-center uppercase">
...
// import { useBookmarkStore } from '@/stores/bookmark.js'; pake ini tidk bisa

const props = defineProps({
    product: Object
});

// Menggunakan state dari store
